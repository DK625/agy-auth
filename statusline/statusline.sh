...
color_for_pct() {
    local pct=$1
    if [ "$pct" -le 10 ] 2>/dev/null; then printf "%b" "$FG_BRIGHT_RED"
    elif [ "$pct" -le 25 ] 2>/dev/null; then printf "%b" "$FG_BRIGHT_YELLOW"
    elif [ "$pct" -le 50 ] 2>/dev/null; then printf "%b" "$FG_BRIGHT_CYAN"
...
format_quota_line() {
    ...
    local pct
    pct=$(LC_NUMERIC=C echo "$rem" | awk '{printf "%.0f", $1 * 100}' 2>/dev/null || echo 0)
    local qBar
