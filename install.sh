...
if command -v curl >/dev/null 2>&1; then
    curl -fsSL "$RAW_URL/bin/agi-auth" -o "$INSTALL_DIR/bin/agi-auth"
elif command -v wget >/dev/null 2>&1; then
    wget -qO "$INSTALL_DIR/bin/agi-auth" "$RAW_URL/bin/agi-auth"
else
...
echo -e "  \033[36magi-auth login <name>\033[0m   - Login to a new account & save as <name>"
echo -e "  \033[36magi-auth list\033[0m           - List all saved accounts"
echo -e "  \033[36magi-auth switch <name>\033[0m  - Switch to saved account"
echo -e "  \033[36magi-auth remove <name>\033[0m  - Remove a saved account"
